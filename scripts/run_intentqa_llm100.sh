...
mkdir -p outputs/intentqa_llm100 logs

echo "===== normalize IntentQA ====="
python datasets/load_mcq.py \
  --input "$ANN" \
  --dataset intentqa \
  --output outputs/intentqa_llm100/input_norm.json \
  --limit 100
...
do
  ...
  python sv_videorag/run_llm_methods.py \
    --input outputs/intentqa_llm100/input_norm.json \
    --docs_json "$DOCS" \
    --method "$METHOD" \
    --output outputs/intentqa_llm100/$METHOD/output.json \
...
python analysis/compare_outputs.py \
  --files \
  outputs/intentqa_llm100/llovi_lite/output.json \
  outputs/intentqa_llm100/videorag_lite/output.json \
  outputs/intentqa_llm100/sv_videorag/output.json \
  --output outputs/intentqa_llm100/summary.json
